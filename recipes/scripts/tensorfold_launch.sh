...
#
# Usage: tensorfold_launch.sh <rank 0|1> <master-ip> [context] [drafter-policy]
...
docker run -d --name "$NAME" \
  ...
  bash -c "pip install -q git+https://github.com/ashhart/TensorFold.git && tensorfold serve Vontra/GLM-5.3-Flash-MLX-4bit-MTP ${EXTRA_ARGS[*]}"

echo "started $NAME (rank $RANK, context=${CONTEXT:-default}, drafter=$DRAFTER)"
