...
def _build_pinned_fixture(guard_anchor: str) -> str:
    """Assemble a fixture module from verbatim excerpts of the live container
    file, pulled read-only, with the given form of the site-3 guard anchor
    substituted in. All three pinned anchors, in file order, in a module that
    compiles.
    """
    return (
        ANCHOR_IMPORT
...
# Current-base (pre-#51718) fixture -- what this fork's base image actually
# ships today. ``PINNED_FIXTURE`` is the pre-existing name every test below
# was already written against; ``ANCHOR_GUARD == ANCHOR_GUARD_OLD`` (see
# patch_indexer_workspace.py's backward-compat aliases), so this is
# byte-identical to what it always was.
PINNED_FIXTURE = _build_pinned_fixture(ANCHOR_GUARD)

# Post-vLLM-PR-#51718 fixture (MLAAttentionSpec.tokens_per_state, not yet
# present in our base image -- see the "DUAL ANCHOR" comment in
# patch_indexer_workspace.py). Only the guard anchor's middle lines differ
# from PINNED_FIXTURE; everything else -- including ANCHOR_IMPORT and
# ANCHOR_SIZE, which #51718 does not touch in this file -- is identical.
PINNED_FIXTURE_NEW = _build_pinned_fixture(ANCHOR_GUARD_NEW)
...
class _MLAAttentionSpec:
    """Stand-in for vllm.v1.kv_cache_interface.MLAAttentionSpec, PRE-#51718.
...
class _MLAAttentionSpecNew:
    """Stand-in for MLAAttentionSpec, POST-#51718 shape.

    Only ``tokens_per_state`` (an int) and the isinstance check matter to the
    new-form guard -- ``compress_ratio`` is deliberately ABSENT here, so a
    guard that still reads the old attribute name would raise AttributeError
    rather than silently falling back to it.
    """

    def __init__(self, tokens_per_state: int):
...
def _patched_sizing_namespace(
    fixture: str = PINNED_FIXTURE, mla_spec_cls: type = _MLAAttentionSpec
) -> dict:
    ...
    ``fixture``/``mla_spec_cls`` let the same harness exercise either anchor
    form: the default (``PINNED_FIXTURE``, ``_MLAAttentionSpec``) is the
    current-base pre-#51718 shape; pass ``PINNED_FIXTURE_NEW`` and
    ``_MLAAttentionSpecNew`` to exercise the post-#51718 shape instead.
    """
    import ast
...
# --------------------------------------------------------------------------
# 4b. dual-anchor hardening (v19-indexercompat, forward-compat for vLLM
#     PR #51718 -- see the "DUAL ANCHOR" comment in patch_indexer_workspace.py)
# --------------------------------------------------------------------------
def test_current_base_patch_is_byte_identical_to_manual_replacement() -> None:
    """The load-bearing behavior-neutrality proof.

    Applying the dual-anchor ``prepare()`` to a current-base (old-anchor)
    file must produce EXACTLY the same bytes as substituting each site's
    single known (anchor -> patched) pair by hand -- the new, post-#51718
    form must never be consulted or leave any trace in the output when the
    old form matches. This is what makes the hardening a pure no-op on the
    base we actually run today.
    """
    from patch_indexer_workspace import PATCHED_SIZE

    patched, action = prepare(PINNED_FIXTURE)
    assert action == "patched"
    expected = PINNED_FIXTURE
    expected = expected.replace(ANCHOR_IMPORT, PATCHED_IMPORT, 1)
    expected = expected.replace(ANCHOR_SIZE, PATCHED_SIZE, 1)
    expected = expected.replace(ANCHOR_GUARD_OLD, PATCHED_GUARD_OLD, 1)
    assert patched == expected
    assert "tokens_per_state" not in patched
    assert "num_states" not in patched


def test_fixture_apply_and_idempotence_new_anchor() -> None:
    """Same lifecycle as ``test_fixture_apply_and_idempotence``, but against
    the post-#51718 (``tokens_per_state``) fixture -- not live-verifiable
    today (no v0.29.0-era container available), so this pins the transcribed
    upstream diff text instead.
    """
    with tempfile.TemporaryDirectory() as raw:
        target = Path(raw) / "indexer.py"
        target.write_text(PINNED_FIXTURE_NEW)

        pre = _run_patch(target, "--preflight")
        assert pre.returncode == 0, pre.stderr
        assert "preflight OK" in pre.stdout
        assert target.read_text() == PINNED_FIXTURE_NEW
        ...
        assert verified_state(text)
        assert MARK_SIZE in text
        assert MARK_GUARD in text
        assert "_glm53_rightsized_workspace_entries" in text
        # The post-#51718 source text survives the patch untouched.
        assert "self.kv_cache_spec.tokens_per_state" in text
        assert "assert isinstance(self.kv_cache_spec.tokens_per_state, int)" in text
        assert "already present" not in first.stdout
        compile(text, str(target), "exec")
...
def test_old_and_new_anchor_produce_identical_append_block() -> None:
    """The injected cross-check DISPATCH code (the part after MARK_GUARD) must
    be byte-identical regardless of which anchor form matched -- it only ever
    reads ``self.compress_ratio`` (the builder's own local attribute, never
    renamed by #51718) and ``self.vllm_config``, never the raw
    ``kv_cache_spec`` attribute name. If a future edit accidentally forked the
    dispatch logic between the two forms, this catches it.
    """
    patched_old, _ = prepare(PINNED_FIXTURE)
    patched_new, _ = prepare(PINNED_FIXTURE_NEW)
    append_old = patched_old[patched_old.index(MARK_GUARD) :]
    append_new = patched_new[patched_new.index(MARK_GUARD) :]
    assert append_old == append_new


def test_dual_anchor_prefers_old_form_when_only_old_present() -> None:
    """When only the current-base form is present, the new form's text must
    not appear anywhere in the patched output (the counter-check to the
    byte-identical test above, phrased as a search instead of a diff).
    """
    patched, _ = prepare(PINNED_FIXTURE)
    assert ANCHOR_GUARD_NEW not in patched
    assert "tokens_per_state" not in patched


def test_builder_ratio_mismatch_raises_both_directions_new_anchor() -> None:
    """``test_builder_ratio_mismatch_raises_both_directions``, replayed
    against the post-#51718 fixture and spec stand-in (``tokens_per_state``,
    no ``compress_ratio`` attribute at all) -- the fail-closed cross-check
    must behave identically regardless of which anchor form applied, since
    both forms funnel into the same ``self.compress_ratio`` local attribute
    before the shared dispatch code (see the append-block-identity test
    above) ever runs.
    """
    ns = _patched_sizing_namespace(
        fixture=PINNED_FIXTURE_NEW, mla_spec_cls=_MLAAttentionSpecNew
    )
    calls = ns["_calls"]
    saved = os.environ.get(ENV_NAME)
    try:
        ...
        sized_for = ns["get_max_prefill_buffer_size"](cfg(kpool=4))
        assert sized_for == 4_000_032
        for spec in (_MLAAttentionSpecNew(1), _NonMLASpec()):
            calls.clear()
            try:
                _build(ns, spec, cfg(kpool=4))
            except ValueError as exc:
                ...
                raise AssertionError(
                    f"index_kpool=4 vs tokens_per_state=1 ({spec}) must raise"
                )

        for kpool in (1, 0, None):
            calls.clear()
            try:
                _build(ns, _MLAAttentionSpecNew(4), cfg(kpool=kpool))
            except ValueError as exc:
                msg = str(exc)
                assert "index_kpool=1" in msg, msg
                assert "compress_ratio=4" in msg, msg
                ...
                raise AssertionError(
                    f"index_kpool={kpool} vs tokens_per_state=4 must raise"
                )
    finally:
        _set_mode(saved)


def test_builder_agreeing_ratios_pass_new_anchor() -> None:
    """``test_builder_agreeing_ratios_pass``, replayed against the
    post-#51718 fixture/spec.
    """
    ns = _patched_sizing_namespace(
        fixture=PINNED_FIXTURE_NEW, mla_spec_cls=_MLAAttentionSpecNew
    )
    ...
    try:
        ...
        calls.clear()
        builder = _build(ns, _MLAAttentionSpecNew(4), cfg(kpool=4))
        assert builder.max_prefill_buffer_size == 4_000_032
        assert [k for k, _ in calls] == ["info", "info"]

        calls.clear()
        builder = _build(ns, _MLAAttentionSpecNew(1), cfg(kpool=1))
        assert builder.max_prefill_buffer_size == stock
        assert [k for k, _ in calls] == ["warning", "info"]
    finally:
        _set_mode(saved)
...
def test_new_anchor_spec_has_no_compress_ratio_attribute() -> None:
    """Sanity check on the stand-in itself: ``_MLAAttentionSpecNew`` must NOT
    define ``compress_ratio`` at all, so a guard that regressed to reading the
    old attribute name on a post-#51718-shaped spec would raise
    AttributeError immediately in the tests above rather than passing by
    accident.
    """
    spec = _MLAAttentionSpecNew(4)
    assert not hasattr(spec, "compress_ratio")
    assert spec.tokens_per_state == 4


def test_fail_closed_when_neither_anchor_form_matches() -> None:
    """The guard site's drift message names BOTH known forms and their
    individual match counts -- not a generic "drifted" message -- so a
    future maintainer immediately knows both a current-base AND a
    post-#51718 form were tried and neither matched.
    """
    garbage = PINNED_FIXTURE.replace(
        "self.compress_ratio = self.kv_cache_spec.compress_ratio",
        "self.compress_ratio = self.kv_cache_spec.something_else_entirely",
        1,
    )
    assert garbage != PINNED_FIXTURE
    with tempfile.TemporaryDirectory() as raw:
        target = Path(raw) / "indexer.py"
        ...
        assert "matched neither known form" in result.stderr
        assert "current-base (compress_ratio) found 0 times" in result.stderr
        assert "post-#51718 (tokens_per_state) found 0 times" in result.stderr
