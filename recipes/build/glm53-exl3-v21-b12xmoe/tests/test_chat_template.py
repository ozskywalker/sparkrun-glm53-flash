...
class NullContentTests(unittest.TestCase):
    """Regression for upstream MiaAI-Lab#156: an assistant turn with
    content=None (tool-calls-only, common in agent traffic) used to render
    the literal string "None" into the prompt via visible_text's unguarded
    {{- content }} branch."""

    def test_tool_call_only_assistant_turn_has_no_literal_none(self) -> None:
        messages = [
        ...
        rendered = render_conversation(messages=messages, tools=TOOLS, enable_thinking=False)
        self.assertNotIn("None", rendered)
        self.assertIn("<tool_call>multiply", rendered)
