...
# consume nearly all of GB10's unified memory and turn an otherwise short
# build into swap thrash. Refuse before docker build while headroom is below
# this threshold. Set 0 only as an explicit operator override.
# Ported from Enntity/sparkglm (a8aaa229) -- an original build-host guard
# derived from an observed GB10 failure mode, not vLLM/upstream code.
...
# Upstream's measured A/B/A at MNBT=7168 with LONG_PREFILL_TOKEN_THRESHOLD=3584
# (MNBT/2): warm-session freeze 131/108 s stock -> 15 s; contended prefill +6%;
# solo prefill +3..+11%, within baseline drift (MiaAI-Lab#157).
# LEFT EMPTY (opt-in) DELIBERATELY: this fleet already ships its own answer
# to the identical "long prefill freezes a warm decode session" problem --
# overlay/patch_scheduler_decode_floor.py's GLM53_MIXED_PREFILL_WARM_TOKENS
# (also defaulted to 3584) / MAX_WAIT_MS / LATE_CAP bypass-and-late-cap
# scheduler patch, same symptom, a different mechanism (our own custom
# scheduler monkeypatch vs. vLLM's native flag here). Enabling both at once
# is untested and could double-throttle or conflict outright -- needs a real
# A/B against our own patch before ever setting this non-empty.
...
count_dflash_shard() {
    ...
    find "$repo_path/snapshots/$ref" -maxdepth 1 -type f -name 'model.safetensors' 2>/dev/null \
        | wc -l | tr -d '[:space:]' || true
}
...
download_dflash() {
    ...
    local have
    # Scoped to the active (refs/main) snapshot, not any snapshot — see
    # count_shards()'s identical fix (upstream MiaAI-Lab#153): a stale
    # snapshots/old with model.safetensors would otherwise satisfy this
    # check even when the snapshot refs/main actually resolves to is empty.
    have="$(count_dflash_shard "$DFLASH_PATH")"
